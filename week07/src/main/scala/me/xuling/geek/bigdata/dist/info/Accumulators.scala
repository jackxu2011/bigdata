...
import me.xuling.geek.bigdata.dist.util.FileUtils
...
class Accumulators(sparkSession: SparkSession) extends Serializable {
  ...
  def getOutputText: String = {
    ...
    s"""--Raw data--
       ...
       |Data skipped (already existing files, dry-run and failures): ${
      FileUtils
        .byteCountToDisplaySize(bytesSkipped.value)
    }
       |--Files--
       |Files copied (new files and overwritten/updated files): ${
      intFormatter
      ...
      intFormatter.format(
       ...
       |Skipped files for copying (already existing files, dry-run and failures): ${
      intFormatter
       ...
       |Skipped folder creates (already existing folders, dry-run and failures): ${
      intFormatter
      ...
      intFormatter.format(
       ...
       |Skipped delete operations (files/folders already missing, dry-run and failures): ${
      intFormatter
